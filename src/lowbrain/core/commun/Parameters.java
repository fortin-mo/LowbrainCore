...
public class Parameters {
    ...
    public Parameters(String path) {
        ...
        if (path == null) {
            ...
            plugin.debugWarning(DEFAULT_PARAMETERS + " will be use as default");
        }

        File file = new File(plugin.getDataFolder(), path);

        if (!file.exists() && path != DEFAULT_PARAMETERS) {
        ...
        if (!file.exists()) {
            ...
            plugin.saveResource(DEFAULT_PARAMETERS, false);
        }
        ...
        this.function_type = config.getInt("function_type");
        this.next_lvl_multiplier = (float)config.getDouble("next_lvl_multiplier");
        this.natural_xp_gain_multiplier = (float)config.getDouble("natural_xp_gain_multiplier");
        this.killer_level_gain_multiplier = (float)config.getDouble("on_player_kills_player.killer_level_gain_multiplier");
        this.level_difference_multiplier = (float)config.getDouble("on_player_kills_player.level_difference_multiplier");
        this.killer_base_exp = (float)config.getDouble("on_player_kills_player.killer_base_exp");
        this.player_kills_player_exp_enable = config.getBoolean("on_player_kills_player.enable");

        onPlayerAttackEntity = new onPlayerAttackEntity(config.getConfigurationSection("on_player_attack_entity"));
        onPlayerConsumePotion = new onPlayerConsumePotion(config.getConfigurationSection("on_player_consume_potion"));
